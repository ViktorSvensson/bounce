 ...
 * @template O Output type
 */
export class Bounce<I = any, O extends any = I> implements BounceModule<I, O> {
  /**
   * Creates an instance of bounce.
   *
   * @param modules
   */
  protected constructor(protected readonly modules: BounceModule[]) {}
  ...
  public apply(input: I): O {
    ...
    for (let i = 0; i < this.modules.length; i++) {
      x = this.modules[i].apply(x as I) as O;
    }
  ...
  public restore(output: O): I {
    ...
    for (let i = this.modules.length - 1; i >= 0; i--) {
      x = this.modules[i].restore(x as O) as I;
    }
  ...
  public static create<I, O>(
    ...modules: BounceModule<I | unknown, O | unknown>[]
  ): Bounce<I, O> {
